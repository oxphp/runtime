...
final class FakeOxRequest implements RequestInterface
{
    ...
    {
        return new self(method: 'GET', path: $path, query: $query,
    ...
    public function queryString(): ?string { return $this->queryString === '' ? null : $this->queryString; }
    public function isSecure(): bool { return $this->scheme === 'https'; }
    public function isMethod(string $method): bool { return \strcasecmp($this->method, $method) === 0; }
    public function httpProtocol(): string { return 'HTTP/'.$this->httpProtocolVersion; }
    public function httpProtocolVersion(): string { return $this->httpProtocolVersion; }
    ...
    {
        if ($key === null) { return $this->payload; }
    ...
    public function cookie(string $name, ?string $default = null): ?string { return $this->cookies[$name] ?? $default; }
    ...
    public function file(string $name): ?UploadedFileInterface
    {
        foreach ($this->files as $f) {
            if ($f instanceof UploadedFileInterface && $f->name() === $name) { return $f; }
        }
        return null;
    }
